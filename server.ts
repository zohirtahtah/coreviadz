...
// -------------------------------------------------------------
// POST /api/auth/register -> Tenant (Company + Owner) Registration
// Uses service_role key to bypass RLS during initial provisioning
// -------------------------------------------------------------
app.post("/api/auth/register", async (req, res) => {
  ...
  try {
    ...
    // Step B: Create company record (bypasses RLS via service_role)
    ...
    // Strip unknown columns gracefully (same pattern as resilientUpsert)
    ...
    // Create user record in corevia_saas_users (bypasses RLS via service_role)
    let saasUserPayload: Record<string, any> = {
      user_id: userId,
      company_id: companyId,
      email: cleanEmail,
    ...
    for (let attempt = 0; attempt < 20; attempt++) {
      const { error: upsertErr } = await supabaseAdmin
        .from("corevia_saas_users")
        .upsert(saasUserPayload, { onConflict: "user_id" });
...
// -------------------------------------------------------------
// POST /api/auth/provision-company -> Creates company+user for OAuth users (Google, etc.)
// Called after successful OAuth when no existing company record is found.
...
app.post("/api/auth/provision-company", async (req, res) => {
  ...
  try {
    ...
    // Upsert company record (bypasses RLS)
    const companyPayload: Record<string, any> = {
      id: companyId,
    ...
    for (let attempt = 0; attempt < 20; attempt++) {
      ...
      else { console.error("Company provision fatal:", upsertErr); break; }
    }

    // Upsert user record (bypasses RLS)
    ...
    for (let attempt = 0; attempt < 20; attempt++) {
      ...
      else { console.error("User provision fatal:", upsertErr); break; }
    }
